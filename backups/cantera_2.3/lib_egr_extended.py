...
def fresh_gas(phi,config,egr_rate):
    gas = ct.Solution(config.scheme)
    gas.TP = config.res.ox.thermo.T,config.res.ox.thermo.P
    gas.set_equivalence_ratio(phi, config.compo.fuel, config.compo.ox)
                              #, basis="mole",
...
def compute_solutions_1D(config,real_egr=False,species = ['CH4','H2','O2','CO','CO2','H2O']):
    ...
    for egr in config.egr_range:
        for phi in config.phi_range:
            f = build_freeflame(fresh_gas(phi,config,egr))
            ...
            df = pd.concat([df, pd.DataFrame([[egr, phi, P, T, f.T[-1], f.u[0]]+list(f.X[index,-1])], columns=['EGR','phi','P','Tin','T','u']+species)]).astype(float) #+list(f.X[index][-1])] #
...
if __name__ == '__main__':
    ...
    for p in pressures:
        ...
        reactor,pdresult = compute_solutions_1D(config,real_egr=False)
