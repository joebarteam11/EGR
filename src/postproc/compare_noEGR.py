...
inputs=[pd.read_csv(file).round({'P':1,'EGR':1,'phi':2}) for file in files]
...
for i,var in enumerate(var_to_plot):
    ...
    for j,input in enumerate(inputs):
        ...
        data=data.loc[:,data.columns.get_level_values('EGR')<0.1]
        print(data)
        
        label = [
                 #r'$\ X_{EGR}^{fuel}$'+':'+str(val[0])+' '
                 #+'('+
                 mech[j%2]
                 #+')'
