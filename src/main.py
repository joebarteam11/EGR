...
if __name__ == '__main__':
    ...
    presslist = [1e5,5e5] #[i for i in np.arange(1e5,5.1e5,5e4)]
    config = case('CH4:1.',                     #fuel compo
                  ...
                  temptlist,                    #tin egr
                  presslist,                        #pin egr
                  [0.8,1.0,1.2],#[i for i in np.arange(0.60,2.51,0.1)],        #phi range
                  [0],#[i for i in np.linspace(0.0,0.6,30)],#[0.0,0.1,0.15,0.2],            #egr range
                  'mole',                       #egr rate unit mole / mass
                  'gri30.cti',               #path to scheme
                  'Mix', #transport model
    ...
    if(dim=='equilibrate'):
        ...
        varstosave = ['phi','AF','P','Tin','T','u','dF']
        species = ['CH4','H2','O2','CO','CO2','H2O']

        #Computation pool 
        pool = mp.Pool(min(len(items),ncpu))
        results = [pool.apply_async(compute_solutions_1D, args=item+[restart_rate,real_egr,species,varstosave],callback=update) for item in items]
        pool.close()
        # wait for all tasks to complete and processes to close
        pool.join()
